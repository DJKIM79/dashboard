...
const shortcutMod = {
  ...
  renderIconList(filter = "", append = false) {
    ...
    if (!append) {
      grid.innerHTML = "";
      grid.scrollTop = 0;
      this.iconListIndex = 0;
      this.filteredIcons = filter 
    ...
    const batchSize = 105; // 15 rows of 7
    const batch = this.filteredIcons.slice(this.iconListIndex, this.iconListIndex + batchSize);
    
    const fragment = document.createDocumentFragment();
    batch.forEach((icon, i) => {
      const globalIndex = this.iconListIndex + i;
      const div = document.createElement("div");
      div.className = "icon-item";
      
      if (globalIndex < 42) {
        div.style.animationDelay = `${globalIndex * 4}ms`;
      }
    ...
    this.iconListIndex += batchSize;

    if (!this.iconScrollAdded) {
      grid.onscroll = () => {
        if (grid.scrollTop + grid.clientHeight >= grid.scrollHeight - 50) {
          if (this.iconListIndex < this.filteredIcons.length) {
            this.renderIconList(filter, true);
          }
        }
      };
      this.iconScrollAdded = true;
    }
  },
  filterIcons(val) {
    if (this.iconFilterTimeout) clearTimeout(this.iconFilterTimeout);
