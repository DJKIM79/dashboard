const ai = {
  ...
  init() {
    ...
    if (allChats.length > 0) {
      ...
      const newChat = { id: this.currentChatId, title: "새 대화", messages: [], model: this.settingsModel };
      this.chats = [newChat];
    }
  ...
  updateModelDisplay() {
    ...
    if (historyTitleEl) historyTitleEl.innerText = (chat?.model || this.settingsModel) || "AI Chat";
  },
  ...
  toggleModelPopup(e) {
    ...
    if (models.length > 1) {
      models.forEach(m => {
        ...
        div.onclick = () => { this.selectTemporaryModel(m); popup.classList.remove('show'); };
  ...
  updateModelSelectUI(models) {
    ...
    if (this.isConnected && models.length > 0) {
      ...
      if (!models.includes(oldModel)) {
        ...
        if (oldModel !== "") {
          setTimeout(() => { this.renderHistory(); this.loadChat(); }, 50);
        }
      }
      modelSelect.innerHTML = models.map(m => `<option value="${m}" ${m === this.settingsModel ? 'selected' : ''}>${m}</option>`).join('');
  ...
  updateStatusUI() {
    ...
    if (this.isConnected) {
      ...
      if (text) {
        let pName = this.provider === 'local' ? "로컬 AI" : this.provider.charAt(0).toUpperCase() + this.provider.slice(1);
        text.innerText = `${pName} 연결됨`;
      }
      if (statusSpan) statusSpan.style.color = green;
    } else {
      const gray = "#94a3b8", red = "#ef4444";
      if (dot) dot.style.background = gray; 
      if (historyDot) historyDot.style.background = (this.provider !== 'none') ? red : gray;
      if (text) text.innerText = (window.i18n ? window.i18n.get("aiNeedConnect") : "서버 연결 안됨") || "서버 연결 안됨";
  ...
  async checkConnection(isSilent = false) {
    ...
    const finalize = (isConnected, models = []) => {
      if (isConnected) { this.updateChatbotAvailability(true); this.updateModelSelectUI(models); }
      else if (!isSilent) { this.updateChatbotAvailability(false); this.updateModelSelectUI([]); }
      if (!isSilent) {
  ...
  async sendMessage() {
    ...
    if (!chat) { this.createNewChat(); chats = this.chats; chat = chats[0]; }
    const activeModel = chat.model || this.settingsModel;
  ...
  async callLocalAI(prompt, msgDiv, chat, model) {
    ...
    const response = await fetch(`${this.serverUrl}/api/chat`, {
      ...
      body: JSON.stringify({ model: model, messages: chat.messages.map(m => ({ role: m.role, content: m.content })).concat([{ role: 'user', content: prompt }]), stream: isStream })
    });
    ...
    if (isStream) {
      ...
      fullText = json.message?.content || "";
      msgDiv.innerHTML = ""; msgDiv.innerText = fullText;
    }
  ...
  async callOpenAI(prompt, msgDiv, chat, model) {
    const res = await fetch("https://api.openai.com/v1/chat/completions", {
      ...
      body: JSON.stringify({ model: model, messages: chat.messages.map(m => ({ role: m.role, content: m.content })).concat([{ role: 'user', content: prompt }]) })
    });
    if (!res.ok) throw new Error();
    const json = await res.json(), text = json.choices[0].message.content;
    msgDiv.innerHTML = ""; msgDiv.innerText = text;
  ...
  async callGemini(prompt, msgDiv, chat, model) {
    ...
    if (!res.ok) throw new Error();
    const json = await res.json(), text = json.candidates[0].content.parts[0].text;
    msgDiv.innerHTML = ""; msgDiv.innerText = text;
    this.saveMessage(chat.id, prompt, text);
  },
  ...
  createNewChat() {
    ...
    if (emptyChat) { this.loadChat(emptyChat.id); return; }
    this.currentChatId = Date.now();
    const newChats = [{ id: this.currentChatId, title: "새 대화", messages: [], model: this.settingsModel }, ...chats];
    this.chats = newChats;
    this.loadChat(this.currentChatId);
  },
  deleteChat(id, e) {
  ...
  attachFile(e) {
    ...
    utils.showValidationTip("ai-attach-wrapper", "파일 첨부 기능은 향후에 지원할 예정입니다.");
  },
  hideAttachTip() {
